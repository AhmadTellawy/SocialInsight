...
module.exports=function createStageAdapter(){
 ...
 async function call(action,method,data,query=''){
  ...
  const response=await fetch(`${publicBase}/admin/${action}${query}`,{method,headers,body,signal:AbortSignal.timeout(12000)});
  if(!response.ok)throw Error(`STAGE_MEDIA_HTTP_${response.status}`);
