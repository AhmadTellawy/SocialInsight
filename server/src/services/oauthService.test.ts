...
test.beforeEach(() => { prisma.$transaction = async (work: any) => work({$executeRaw: async () => {}, user: prisma.user, oAuthAccount: prisma.oAuthAccount}); });
...
test('explicit linking rejects a provider identity already linked to another user', async () => {
  ...
  installState(stateRecord('GOOGLE', state, { mode: 'LINK', linkingUserId: 'current-user', sessionId: 'link-session' }));
  OAuth2Client.prototype.getToken = async () => ({ tokens: { id_token: 'id-token-fixture' } } as any);
  ...
  prisma.$transaction = async (work: any) => work({ $executeRaw: async () => {}, user: {findUnique: async () => ({id: 'current-user', status: 'ACTIVE'})}, authSession: {findFirst: async () => ({id: 'link-session', recentAuthenticatedAt: new Date()})}, oAuthAccount: prisma.oAuthAccount });
  await assert.rejects(completeOAuth('GOOGLE', 'authorization-code', state, BROWSER_SECRET, 'current-user', 'link-session'), (error: any) => error.code === 'OAUTH_ACCOUNT_CONFLICT');
});
...
test('OAuth cannot bless a provider link removed between initial resolution and the account lock', async () => {
  for (const mode of ['LOGIN', 'REAUTH']) {
    const state = `unlink-interleaving-${mode}`;
    installState(stateRecord('GOOGLE', state, {mode, linkingUserId: mode === 'REAUTH' ? 'current-user' : null, sessionId: mode === 'REAUTH' ? 'current-session' : null}));
    OAuth2Client.prototype.getToken = async () => ({tokens:{id_token:'id-token-fixture'}} as any);
    OAuth2Client.prototype.verifyIdToken = async () => ({getPayload:()=>({sub:'google-user',nonce:'nonce-fixture',email:'private@example.test',email_verified:true})} as any);
    prisma.oAuthAccount.findFirst = async () => ({userId:'current-user',providerAccountId:'google-user'});
    let userRead = false;
    prisma.$transaction = async (work: any) => work({$executeRaw:async()=>{},oAuthAccount:{findFirst:async()=>null},user:{findUnique:async()=>{userRead=true;return{id:'current-user',status:'ACTIVE',authInvalidatedAt:new Date()};}}});
    await assert.rejects(completeOAuth('GOOGLE','code',state,BROWSER_SECRET,mode==='REAUTH'?'current-user':undefined,mode==='REAUTH'?'current-session':undefined),(error:any)=>error.code==='OAUTH_REAUTH_IDENTITY_INVALID');
