...
export const beginOAuth = async (provider: OAuthProvider, mode: OAuthMode, linkingUserId?: string, sessionId?: string): Promise<OAuthStartResult> => {
    if (mode !== 'LOGIN' && (!linkingUserId || !sessionId)) throw new OAuthError('AUTH_REQUIRED');
    ...
    if (provider === 'GOOGLE') {
        ...
        const params = new URLSearchParams({
            ...
            ...(mode === 'REAUTH' ? { auth_type: 'reauthorize' } : {}),
...
const googleIdentity = async (code: string, stateRecord: any): Promise<ProviderIdentity> => {
    ...
    return {
        ...
        emailVerified: true,
        name: typeof payload.name === 'string' && payload.name.trim() ? payload.name.trim().slice(0, 100) : 'Opiniup User'
    };
};
...
const facebookIdentity = async (code: string, stateRecord: any): Promise<ProviderIdentity> => {
    ...
    return {
        ...
        emailVerified: false,
        name: typeof profile.name === 'string' && profile.name.trim() ? profile.name.trim().slice(0, 100) : 'Opiniup User'
    };
};
...
const resolveLinkedIdentity = async (identity: ProviderIdentity, userId: string): Promise<any> => db.$transaction(async (tx: any) => {
    await lockAccountSecurity(tx, userId);
    const linked = await tx.oAuthAccount.findFirst({ where: { provider: identity.provider, providerAccountId: identity.providerAccountId, userId } });
    if (!linked || linked.userId !== userId) throw new OAuthError('OAUTH_REAUTH_IDENTITY_INVALID');
    // Return the credential epoch from this same lock, never bless a provider assertion with a newer post-unlink user snapshot.
...
const resolveIdentity = async (identity: ProviderIdentity, stateRecord: any): Promise<any> => {
    ...
    // the canonical Opiniup email or participate in email-collision
    ...
    if (stateRecord.mode === 'REAUTH') {
        if (!linked || linked.userId !== stateRecord.linkingUserId) throw new OAuthError('OAUTH_REAUTH_IDENTITY_INVALID');
        return resolveLinkedIdentity(identity, linked.userId);
    }
    if (stateRecord.mode === 'LINK') {
        if (!stateRecord.linkingUserId) throw new OAuthError('AUTH_REQUIRED');
        return db.$transaction(async (tx: any) => {
            await lockAccountSecurity(tx, stateRecord.linkingUserId);
            const user = await tx.user.findUnique({ where: { id: stateRecord.linkingUserId } });
            if (user?.status !== 'ACTIVE') throw new OAuthError('AUTH_ACCOUNT_INACTIVE');
            const liveSession = await tx.authSession.findFirst({ where: { id: stateRecord.sessionId, userId: user.id, revokedAt: null, expiresAt: { gt: new Date() } } });
            const ttl = Math.max(60, Math.min(3600, Number(process.env.AUTH_RECENT_TTL_SECONDS) || 600));
            if (!liveSession || Date.now() - (liveSession.recentAuthenticatedAt || liveSession.createdAt).getTime() > ttl * 1000) throw new OAuthError('OAUTH_LINK_SESSION_INVALID');
            const existing = await tx.oAuthAccount.findFirst({ where: { provider: identity.provider, providerAccountId: identity.providerAccountId } });
            if (existing && existing.userId !== user.id) throw new OAuthError('OAUTH_ACCOUNT_CONFLICT');
            const providerLink = await tx.oAuthAccount.findFirst({ where: { userId: user.id, provider: identity.provider } });
            if (providerLink && providerLink.providerAccountId !== identity.providerAccountId) throw new OAuthError('OAUTH_PROVIDER_ALREADY_LINKED');
            if (trustedEmail) {
                const emailOwner = await tx.user.findFirst({ where: { email: { equals: trustedEmail, mode: 'insensitive' } }, select: { id: true } });
                if (emailOwner && emailOwner.id !== user.id) throw new OAuthError('OAUTH_ACCOUNT_CONFLICT');
            }
            if (!existing) await tx.oAuthAccount.create({ data: { userId: user.id, provider: identity.provider, providerAccountId: identity.providerAccountId, emailSnapshot: identity.email } });
            return user;
        });
    }

    if (linked) return resolveLinkedIdentity(identity, linked.userId);
    if (trustedEmail) {
...
export const completeOAuth = async (
    ...
    if (stateRecord.mode !== 'LOGIN' && (!callbackSessionUserId || callbackSessionUserId !== stateRecord.linkingUserId || (!stateRecord.sessionId || callbackSessionId !== stateRecord.sessionId))) {
    ...
    try {
        ...
        if (!user || !['ACTIVE', ...(stateRecord.mode === 'LOGIN' ? ['DEACTIVATED'] : [])].includes(user.status)) throw new OAuthError('AUTH_ACCOUNT_INACTIVE');
