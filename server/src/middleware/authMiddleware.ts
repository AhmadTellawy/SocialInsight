...
export const requireAuth = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (legacyUserId && session && legacyUserId !== session.userId) {
        res.status(401).json({ error: 'Authentication identities differ', code: 'AUTH_IDENTITY_MISMATCH' });
        return;
    }
    if (legacyUserId) {
...
export const requireRecentAuth = (req: Request, res: Response, next: NextFunction): void => {
    ...
    const createdAt = req.authSession?.recentAuthenticatedAt || req.authSession?.createdAt;
    if (req.user?.authMode !== 'session' || req.authSession?.userId !== req.user?.userId || !createdAt || Date.now() - createdAt.getTime() > ttlSeconds * 1000) {
        res.status(401).json({
