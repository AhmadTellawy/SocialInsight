...
test('sensitive sign-in method changes require a recently created server session', () => {
  ...
  let nextCalls = 0;
  requireRecentAuth({ user: { userId: 'a', authMode: 'session' }, authSession: { userId: 'a', createdAt: new Date() } } as any, fresh.res, () => { nextCalls += 1; });
  assert.equal(nextCalls, 1);

  const stale = response();
  requireRecentAuth({ user: { userId: 'a', authMode: 'session' }, authSession: { userId: 'a', createdAt: new Date(Date.now() - 60 * 60 * 1000) }, requestId: 'stale' } as any, stale.res, () => { nextCalls += 1; });
