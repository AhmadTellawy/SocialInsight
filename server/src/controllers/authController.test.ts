...
const authChallengeService = require('../services/authChallengeService') as typeof import('../services/authChallengeService');
...
const originals = {
  primaryChallenge: authChallengeService.challengeAfterPrimaryProof,
  mfaFindUnique: prisma.userMfa.findUnique,
...
test.beforeEach(() => { (authChallengeService as any).challengeAfterPrimaryProof = async () => null; prisma.userMfa.findUnique = async () => null; });

test.afterEach(() => {
  (authChallengeService as any).challengeAfterPrimaryProof = originals.primaryChallenge;
  prisma.userMfa.findUnique = originals.mfaFindUnique;
...
test('registration completion consumes a purpose-bound OTP, verifies email and starts an opaque session', async () => {
  ...
  (otpService as any).consumeEmailOtp = async (input: any, onConsume: any) => {
    consumed = input;
    return { challengeId: 'challenge-1', value: await prisma.$transaction((tx: any) => onConsume(tx, {createdAt: new Date()})) };
  };
  let createdData: any;
...
test('password reset consumes bound OTP, updates hash and revokes all sessions atomically', async () => {
  ...
  (otpService as any).consumeEmailOtp = async (input: any, onConsume: any) => {
    consumed = input;
    return { challengeId: 'challenge-reset', value: await prisma.$transaction((tx: any) => onConsume(tx, {createdAt: new Date()})) };
  };
...
test('a password reset OTP cannot cross a concurrent email or credential change', async () => {
  prisma.user.findFirst = async () => ({id:'user-1',status:'ACTIVE'});
  (bcrypt as any).hash = async () => 'new-password-hash';
  const issuedAt = new Date('2026-09-07T12:00:00Z');
  for (const current of [
    {email:'new-owner@example.test',status:'ACTIVE',authInvalidatedAt:null},
    {email:'private@example.test',status:'ACTIVE',authInvalidatedAt:new Date(issuedAt.getTime()+1000)}
  ]) {
    let writes = 0;
    const tx = {$executeRaw:async()=>{},user:{findUnique:async()=>current,update:async()=>{writes++;}},otpChallenge:{updateMany:async()=>{writes++;}},authSession:{updateMany:async()=>{writes++;}}};
    (otpService as any).consumeEmailOtp = async (_input:any,onConsume:any) => ({challengeId:'old-proof',value:await onConsume(tx,{createdAt:issuedAt})});
    const {response,state}=createResponse();
    await confirmPasswordReset({body:{email:'private@example.test',code:'123456',password:'NewPassword1!'}} as any,response);
    assert.equal(state.statusCode,400);assert.equal(state.body.code,'PASSWORD_RESET_INVALID');assert.equal(writes,0);
  }
});
...
test('email verification request/confirm uses current account destination and correct purpose', async () => {
  ...
  (otpService as any).consumeEmailOtp = async (input: any, onConsume: any) => {
    consumed = input;
    return { challengeId: 'challenge-verify', value: await prisma.$transaction((tx: any) => onConsume(tx, {createdAt: new Date()})) };
  };
  prisma.$transaction = async (callback: any) => callback({ $executeRaw: async () => {}, authSession: {findFirst: async () => ({id: 'session-old', recentAuthenticatedAt: new Date()})}, user: { findUnique: async () => ({email: 'private@example.test', status: 'ACTIVE', authInvalidatedAt: null}), update: async (input: any) => { update = input; return {}; } } });
  const req: any = { user: { userId: 'user-1', authMode: 'session' }, authSession: {id: 'session-old', userId: 'user-1'}, body: {}, ip: '127.0.0.1', header: () => undefined, requestId: 'verify-email' };
