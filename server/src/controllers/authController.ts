...
const completeRegistrationSchema = pendingSchema.extend({ otp: z.string().transform(normalizeSecurityCode).pipe(z.string().regex(/^\d{6}$/)).optional(), code: z.string().transform(normalizeSecurityCode).pipe(z.string().regex(/^\d{6}$/)).optional() })
...
const passwordResetConfirmSchema = emailSchema.extend({ code: z.string().transform(normalizeSecurityCode).pipe(z.string().regex(/^\d{6}$/)), password: passwordSchema });
const codeSchema = z.object({ code: z.string().transform(normalizeSecurityCode).pipe(z.string().regex(/^\d{6}$/)) });
const emailChangeConfirmSchema = emailSchema.extend({ code: z.string().transform(normalizeSecurityCode).pipe(z.string().regex(/^\d{6}$/)) });

export const SAFE_USER_SELECT = {
    id: true, name: true, handle: true, email: true, emailVerifiedAt: true, avatar: true,
    ...PUBLIC_AVATAR_MEDIA_SELECT,
    country: true, bio: true, location: true, website: true, language: true, theme: true, isPrivate: true, searchVisibility: true, allowSharing: true, groupInvites: true, groupPrivacy: true, peopleTagPermission: true,
...
const publicUser = (user: any): any => {
    ...
    // Select by contract, never serialize authentication metadata from a full user row.
    const safe = Object.fromEntries(Object.keys(SAFE_USER_SELECT).filter((key) => key in serialized).map((key) => [key, serialized[key]]));
    for (const key of ['avatarMedia', 'avatarMediaId', 'coverMedia', 'coverMediaId']) if (key in serialized) safe[key] = serialized[key];
    return {
        ...safe,
...
export const authenticationResponse = (user: any, csrfToken: string): Record<string, unknown> => {
...
export const login = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (!user || !['ACTIVE', 'DEACTIVATED'].includes(user.status) || !valid) {
            noStore(res);
            res.status(401).json({ error: GENERIC_LOGIN_ERROR, code: 'INVALID_CREDENTIALS' });
            return;
        }
        const challenge = await challengeAfterPrimaryProof(user, res);
        if (challenge) { noStore(res); res.json(challenge); return; }
        const demographics = await prisma.userDemographics.findUnique({ where: { userId: user.id } });
        const session = await createSession(user.id, res, req, { expectedAuthInvalidatedAt: user.authInvalidatedAt, expectedPasswordUpdatedAt: user.passwordUpdatedAt });
        noStore(res);
...
export const requestPasswordReset = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        void (async () => {
            if (user && ['ACTIVE', 'DEACTIVATED'].includes(user.status) && user.passwordHash) {
    ...
    try {
        ...
        if (!user.emailVerifiedAt) {
            await consumeEmailOtp({ destination: user.email, purpose: 'EMAIL_VERIFICATION', subject: req.user!.userId, code: parsed.data.code }, async (tx) => {
                ...
                await tx.user.update({ where: { id: req.user!.userId, status: 'ACTIVE', email: user.email }, data: { emailVerifiedAt: new Date() } });
            });
        }
        res.json({ success: true });
    } catch (error) {
...
export const confirmEmailChange = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const session = await createSession(req.user!.userId, res, req, { mfaVerified: true, ...consumed.value });
        noStore(res);
...
export const startOAuthLink = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!provider) return validationFailure(res, 'OAUTH_PROVIDER_INVALID');
    try {
        const started = await beginOAuth(provider, 'LINK', req.user!.userId, req.authSession!.id);
        appendResponseCookie(res, oauthBrowserCookie(started.browserSecret, started.maxAgeSeconds));
        noStore(res);
        res.json({ authorizationUrl: started.authorizationUrl });
    } catch (error) {
...
export const startOAuthReauthentication = async (req: Request, res: Response): Promise<void> => {
    const provider = parseProvider(req.params.provider);
    if (!provider) return validationFailure(res, 'OAUTH_PROVIDER_INVALID');
    if (!req.authSession || req.user?.authMode !== 'session') { res.status(401).json({ code: 'AUTH_REQUIRED' }); return; }
    try {
        const linked = await db.oAuthAccount.findFirst({ where: { userId: req.user.userId, provider }, select: { id: true } });
        if (!linked) { res.status(409).json({ code: 'OAUTH_METHOD_NOT_LINKED' }); return; }
        const started = await beginOAuth(provider, 'REAUTH', req.user.userId, req.authSession.id);
        appendResponseCookie(res, oauthBrowserCookie(started.browserSecret, started.maxAgeSeconds));
        noStore(res);
        res.json({ authorizationUrl: started.authorizationUrl });
    } catch (error) {
        logFailure(req, 'oauth_reauthentication_start_failed', error);
        res.status(503).json({ error: 'OAuth is unavailable', code: 'OAUTH_UNAVAILABLE' });
    }
};
...
export const oauthCallback = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const result = await completeOAuth(provider, code, state, browserSecret, callbackSession?.userId, callbackSession?.id);
        noStore(res);
        if (result.mode === 'LINK') {
            appendResponseCookie(res, oauthBrowserCookie('', 0));
        ...
        if (result.mode === 'REAUTH') {
            const challenge = await challengeAfterPrimaryProof(result.user, res, callbackSession!.id);
            if (!challenge) await db.$transaction(async (tx: any) => {
                await lockAccountSecurity(tx, result.user.id);
                const current = await tx.user.findUnique({ where: { id: result.user.id }, select: { status: true, authInvalidatedAt: true, mfa: { select: { enabledAt: true } } } });
                if (current?.status !== 'ACTIVE' || current.mfa?.enabledAt || (current.authInvalidatedAt?.getTime() ?? null) !== (result.user.authInvalidatedAt?.getTime() ?? null)) throw new OAuthError('OAUTH_REAUTH_IDENTITY_INVALID');
                const updated = await tx.authSession.updateMany({ where: { id: callbackSession!.id, userId: result.user.id, revokedAt: null, expiresAt: { gt: new Date() } }, data: { recentAuthenticatedAt: new Date() } });
                if (updated.count !== 1) throw new OAuthError('OAUTH_LINK_SESSION_INVALID');
            });
            appendResponseCookie(res, oauthBrowserCookie('', 0));
            res.redirect(302, oauthClientRedirect({ oauth: challenge ? 'reauth_challenge' : 'reauthenticated', oauth_provider: provider.toLowerCase() }));
            return;
        }
        const challenge = await challengeAfterPrimaryProof(result.user, res);
        if (challenge) {
            appendResponseCookie(res, oauthBrowserCookie('', 0));
            res.redirect(302, oauthClientRedirect({ oauth: 'challenge', oauth_provider: provider.toLowerCase() }));
            return;
        }
        await createSession(result.user.id, res, req, { expectedAuthInvalidatedAt: result.user.authInvalidatedAt, expectedPasswordUpdatedAt: result.user.passwordUpdatedAt });
        appendResponseCookie(res, oauthBrowserCookie('', 0));
