...
test('protected-action proof failures preserve the session while AUTH_REQUIRED expires it', async () => {
  const events: string[] = [];
  ...
  Object.defineProperty(globalThis, 'fetch', { configurable: true, value: async () => Response.json({ user: { id: 'proof-user' }, csrfToken: 'proof-csrf-fixture-at-least-16' }) });
  await api.login({ identifier: 'proof@example.invalid', password: 'FixturePass1!' });
  for (const code of ['REAUTHENTICATION_REQUIRED', 'INVALID_CREDENTIALS', 'MFA_CODE_INVALID', 'AUTH_CHALLENGE_EXPIRED']) {
    Object.defineProperty(globalThis, 'fetch', { configurable: true, value: async () => Response.json({ code }, { status: 401 }) });
    const response = await authFetch('/api/account/export');
    assert.equal((await response.json()).code, code);
    assert.equal(sessionValues.get('si_auth_identity'), 'proof-user');
  }
  assert.deepEqual(events, []);
  Object.defineProperty(globalThis, 'fetch', { configurable: true, value: async () => Response.json({ code: 'AUTH_REQUIRED' }, { status: 401 }) });
  await (await authFetch('/api/account/export')).json();
  assert.deepEqual(events, ['auth_expired']);
