...
export const authFetch = async (input: RequestInfo | URL, init?: AuthFetchInit): Promise<Response> => {
    ...
    if (response.status === 401 && !suppressAuthExpired) {
        // A fresh proof is required for this action, but the session itself is
        // still valid. Preserve it so the protected-action dialog can resume.
        const payload = await response.clone().json().catch(() => ({}));
        if (!['REAUTHENTICATION_REQUIRED', 'INVALID_CREDENTIALS', 'MFA_CODE_INVALID', 'AUTH_CHALLENGE_EXPIRED', 'AUTH_PROOF_EXPIRED'].includes(payload?.code)) {
...
export const api = {
    ...
    getAuthChallenge: async () => {
        const response = await authFetch(`${API_BASE_URL}/auth/challenge`, { suppressAuthExpired: true });
        if (!response.ok) await throwApiError(response, 'Sign-in verification expired. Sign in again.');
        return response.json();
    },

    completeAuthChallenge: async (data: { code?: string; reactivate?: true }) => {
        const response = await authFetch(`${API_BASE_URL}/auth/challenge/complete`, {
            method: 'POST', body: JSON.stringify(data), suppressAuthExpired: true
        });
        if (!response.ok) await throwApiError(response, 'Could not complete sign-in verification');
    ...
    unsubscribeFromPush: async (endpoint: string) => {
        if (!endpoint) throw new Error('A device subscription is required');
    ...
    setupPushNotifications: async () => {
        ...
        try {
            ...
            const registration = await Promise.race([
                ...
                new Promise<never>((_, reject) => window.setTimeout(() => reject(new Error('Service worker unavailable')), 10_000))
            ]);
            const res = await authFetch(`${API_BASE_URL}/push/vapid-public-key`);
            if (!res.ok) throw new Error('Failed to fetch vapid key');
            const { publicKey } = await res.json();
            if (typeof publicKey !== 'string' || !publicKey) throw new Error('Push is not configured');
            ...
            const saved = await authFetch(`${API_BASE_URL}/push/subscribe`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ subscription })
            });
