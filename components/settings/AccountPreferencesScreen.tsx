  ...
  const save = async (changes: Partial<UserProfile>) => {
    ...
    try {
      ...
      try {
        ...
        update.current(next);
        setNeedsReload(false);
        const confirmed = Object.entries(changes).every(([key, value]) => next[key as keyof UserProfile] === value);
        if (confirmed) {
          if (changes.language) await i18n.changeLanguage(changes.language);
          setSaved(true);
          setSaveError(false);
