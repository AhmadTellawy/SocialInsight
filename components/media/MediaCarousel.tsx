...
export const MediaCarousel: React.FC<MediaCarouselProps> = ({ media, className = '', eager = false, onClick }) => {
  ...
  const aspectRatio = items[0]?.aspectRatio || 1;
  ...
  return (
    <div className={`group relative w-full overflow-hidden bg-gray-100 ${className}`} style={{ aspectRatio }} onClick={onClick}>
      <div
        ref={containerRef}
      ...
      >
        {items.map((item, itemIndex) => (
          <div key={item.id} className="h-full w-full shrink-0 snap-start">
      ...
      {items.length > 1 && (
        <>
          ...
          <div className="pointer-events-none absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5 rounded-full bg-black/45 px-2 py-1" aria-label={`${index + 1} / ${items.length}`}>
            {items.map((item, dotIndex) => <span key={item.id} className={`h-1.5 w-1.5 rounded-full ${dotIndex === index ? 'bg-white' : 'bg-white/45'}`} />)}
          </div>
        </>
      )}
    </div>
