...
export const AccountAccessScreen: React.FC<AccountAccessScreenProps> = ({
  ...
  const reloadMethods = async () => { try { setMethods(await accountRequest<SignInMethods>('/auth/methods')); } catch (e) { setError(securityErrorText(e, ar)); } };
  useEffect(() => { void reloadMethods(); }, []);
  ...
  const confirmVerification = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const updated = { ...userProfile, ...await api.getMe() };
  ...
  const requestChange = async (event: React.FormEvent) => {
    ...
    setNotice(null);
    try {
      if (!await run(async () => { await api.requestEmailChange(normalizedEmail); })) return;
  ...
  const confirmChange = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      let result: Awaited<ReturnType<typeof api.confirmEmailChange>> | undefined;
      if (!await run(async () => { result = await api.confirmEmailChange(nextEmail, code); }) || !result) return;
      const updated = { ...userProfile, ...await api.getMe() };
      onUpdateProfile(updated);
      setFlow(null);
      setCode('');
  ...
  const startLink = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      if (await run(async () => { authorizationUrl = await api.startOAuthLink(provider); })) window.location.assign(authorizationUrl);
      else setBusy(null);
    } catch (linkError) {
  ...
  const resendCurrentCode = flow === 'verify'
    ...
    : async () => {
        ...
        setError(null);
        try {
          if (!await run(async () => { await api.requestEmailChange(nextEmail); })) return;
          setCode('');
          setCooldown(CODE_COOLDOWN_SECONDS);
      ...
      <main className="flex-1 space-y-5 overflow-y-auto px-5 py-6 pb-12">
        ...
        <section aria-labelledby="linked-accounts-title" className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {!methods && <button type="button" className="min-h-11 text-blue-600" onClick={reloadMethods}>{ar ? 'تحميل وسائل الدخول / إعادة المحاولة' : 'Load sign-in methods / retry'}</button>}
            {methods?.providers.map(({provider, linked}) => <div key={provider} className="space-y-2">
              <p className="text-sm text-gray-600">{provider === 'google' ? 'Google' : 'Facebook'}: {linked ? (ar ? 'مرتبط' : 'Linked') : (ar ? 'غير مرتبط' : 'Not linked')}</p>
              {linked ? <button type="button" disabled={Boolean(busy)} className="min-h-11 w-full rounded-xl border p-3 text-red-700 disabled:opacity-50" onClick={async () => {
                setBusy(provider); setError(null); setNotice(null);
                try { if (await run(async () => { await accountRequest('/auth/methods/' + provider, 'DELETE'); })) { await reloadMethods(); setNotice(ar ? 'تم إلغاء ربط وسيلة الدخول.' : 'Sign-in method unlinked.'); } }
                catch (e) { setError(securityErrorText(e, ar)); } finally { setBusy(null); }
              }}>{ar ? 'إلغاء الربط' : 'Unlink'}</button> : <ProviderButton provider={provider} busy={busy} onClick={startLink} label={t('auth.account.oauth.' + (provider === 'google' ? 'linkGoogle' : 'linkFacebook'))} />}
            </div>)}
            <p className="text-xs text-gray-500">{ar ? 'يجب أن تبقى وسيلة دخول واحدة على الأقل مرتبطة بالحساب.' : 'At least one sign-in method must remain available.'}</p>
          </div>
        </section>
      </main>
      {dialog}
