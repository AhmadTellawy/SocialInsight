...
export const PostVisibilitySection: React.FC<Props> = ({ value, onChange, selectedGroupIds, onGroupsChange, groups, allowCustomAudience, allowCustomDomain, error, pagePublisher = false, accent = 'blue' }) => {
  ...
  const changeDestination = (id: string) => {
    if (pagePublisher && id === 'Groups') return;
    if (id === 'Public') {
  ...
  const destinations = [
    { id: 'Public', label: pagePublisher ? 'Page' : 'My Profile', icon: UserRound, selected: profile, allowed: true },
  ...
  return <section aria-labelledby="post-visibility-heading" className="border-y border-gray-100 py-4 space-y-3">
    ...
    <p className="text-xs text-gray-500">{pagePublisher ? 'Publish on your Page. Group publishing uses your personal account.' : 'Keep at least one destination selected. You can select your profile and groups together.'}</p>
    <div className="divide-y divide-gray-100">
      ...
      {destinations.filter(destination => !pagePublisher || destination.id !== 'Groups').map(({ id, label, icon: Icon, selected, allowed }) => id === 'Groups' ? <div key={id} className="flex items-center gap-3 min-h-[48px]">
