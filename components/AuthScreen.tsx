...
import { AuthenticationChallenge, LoginChallenge } from './AuthenticationChallenge';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({
    ...
    useEffect(() => {
        let active = true;
        // The server decides whether a cookie-bound pre-authentication challenge exists.
        void api.getAuthChallenge().then(result => { if (active && result.challenge) setChallenge(result.challenge); }).catch(() => {});
        return () => { active = false; };
    }, []);
    ...
    const handleLoginSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            setPassword('');
            if (result.challengeRequired) setChallenge(result.challenge);
            else onAuthSuccess(result);
        } catch (err) {
    ...
    if (challenge) return <AuthenticationChallenge challenge={challenge} onSuccess={onAuthSuccess} onCancel={() => { setChallenge(null); showView('login'); }} />;
    if (viewMode === 'flow') {
