...
const effectReplacement = `let opts = [...(s.options && s.options.length > 0 ? s.options : (s.type === SurveyType.QUIZ && s.sections ? s.sections.flatMap(sec => sec.questions || [])[0]?.options || [] : []))];`;
...
const renderReturnRegex = /return \(\s*<div className="mt-2">\s*\{hasAddedCustomOption/m;
