...
function installCreateTracking(page: Page, state: CreateTrackingState): void {
  ...
  page.on('response', (response) => {
    ...
    if (!isCreateRequest(request)) return;
    ...
    if (locationId) state.idCaptured = locationId;
...
async function waitForCreateResponse(state: CreateTrackingState, mutationState: MutationState): Promise<void> {
  const deadline = Date.now() + 30_000;

  while (Date.now() <= deadline) {
    const blocked = mutationsByAction(mutationState, 'blocked')[0];
    if (blocked) {
      throw new Error(`Blocked mutation before create response: ${mutationSummary([blocked]).join('')}`);
    }

    if (state.responseObserved && state.responseParsed) return;
    if (state.requestFailed) throw new Error(`Create request failed: ${state.requestFailureText || 'request-failed'}`);
  ...
  throw new Error(
    [
      ...
      `requestObserved=${state.requestObserved ? 'yes' : 'no'}`,
      `responseObserved=${state.responseObserved ? 'yes' : 'no'}`,
      `requestFinished=${state.requestFinished ? 'yes' : 'no'}`,
      `requestFailed=${state.requestFailed ? 'yes' : 'no'}`,
      `idCaptured=${state.idCaptured ? 'yes' : 'no'}`,
      `neutralized=${mutationsByAction(mutationState, 'neutralized').length}`,
      `blocked=${mutationsByAction(mutationState, 'blocked').length}`,
    ].join('; '),
  );
}

function pageDelay(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

async function cleanupCreatedPost(page: Page, state: MutationState): Promise<void> {
...
test.describe('ONLINE_CONTROLLED_WRITE basic text post creation', () => {
  test.skip(!controlledWriteApproved, `ONLINE_CONTROLLED_WRITE requires ${CONTROLLED_WRITE_APPROVAL_ENV}=true.`);

  test('creates and cleans up a basic text post', async ({ page }) => {
    ...
    const textPostTitle = `e2e_text_post_${uniqueSuffix}`;
    ...
    expect(textPostTitle.startsWith('e2e_')).toBe(true);
    ...
    try {
      ...
      expect(createState.idCaptured, 'expected created text post ID').toBeTruthy();
      ...
      await cleanupCreatedPost(page, state);
    } finally {
      console.log(`TEXT_POST_CONTROLLED_WRITE_MUTATIONS ${JSON.stringify(mutationSummary(state.mutations))}`);
      console.log(`TEXT_POST_CREATE_TRACKING ${JSON.stringify({
    ...
    expect(state.createRequests, 'expected exactly one text post create request').toBe(1);
    expect(state.cleanupRequests, 'expected exactly one text post cleanup request').toBe(1);
