...
test('retains a consumed nonce through the entire inclusive timestamp validity second', () => {
  const replayGuard = new ReplayGuard();
  const headers = { 'x-si-timestamp': timestamp, 'x-si-request-id': requestId, 'x-si-signature': signRequest({ secret, timestamp, requestId, body }) };
  authenticateRequest({ headers, body, secret, nowMs, windowSeconds: 300, replayGuard });
  for (const offset of [300000, 300500, 300999]) {
    assert.throws(() => authenticateRequest({ headers, body, secret, nowMs: nowMs + offset, windowSeconds: 300, replayGuard }), { code: 'REPLAYED_REQUEST' });
  }
  assert.throws(() => authenticateRequest({ headers, body, secret, nowMs: nowMs + 301000, windowSeconds: 300, replayGuard }), { code: 'INVALID_SIGNATURE' });
});
