...
assert(dirname(resultPath) === here && !existsSync(resultPath), 'RESULT_MUST_BE_NEW_OWNED_FILE');
...
const { connectionContract, childEnvironment, PROJECT } = await import(pathToFileURL(contractPath));
const receipt = {
...
async function runCase(name, certName, caName, refuseTls) {
  ...
  const obs = { connections: 0, sslRequests: 0, tlsHandshakes: 0, tlsErrors: 0, startupMessages: 0, startupIdentityMatched: false, passwordMessages: 0, passwordMatched: false, queryMessages: 0, targetQueryMatched: false, unexpectedMessages: 0, plaintextAfterTlsRefusalBytes: 0, tlsProtocols: [], observedMessageTags: [], queryKinds: [] };
    ...
    const initialListener = chunk => {
      ...
      peer.on('data', data => {
        ...
        if (startup) {
          ...
          obs.startupIdentityMatched = startupBytes.readUInt32BE(4) === 196608 && fields.get('user') === `postgres.${PROJECT}` && fields.get('database') === 'postgres';
          peer.write(auth(3));
        }
  ...
  const conn = connectionContract({ STAGING_INITIAL_INSTALL_PROJECT: PROJECT, STAGING_DB_TRANSPORT: 'session', STAGING_DB_ADMIN_PASSWORD: syntheticPassword, STAGING_DB_CA_FILE: caFile, STAGING_DB_CA_SHA256: sha(caBytes) }, caBytes);
...
try {
  fixture = mkdtempSync(resolve(here, 'fixture-')); fixtureReal = realpathSync(fixture);
  assert(dirname(fixtureReal) === realpathSync(here) && basename(fixtureReal).startsWith('fixture-'), 'FIXTURE_OUTSIDE_OWNERSHIP');
  ...
  if (fixtureReal) {
    assert(realpathSync(fixture) === fixtureReal && dirname(fixtureReal) === realpathSync(here) && basename(fixtureReal).startsWith('fixture-'), 'CLEANUP_TARGET_INVALID');
